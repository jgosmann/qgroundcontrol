...
typedef struct __mavlink_huch_sensor_array_t
{
 ...
 float data[16]; ///< 16-vector of generic double
} mavlink_huch_sensor_array_t;

#define MAVLINK_MSG_ID_HUCH_SENSOR_ARRAY_LEN 72
...
#define MAVLINK_MESSAGE_INFO_HUCH_SENSOR_ARRAY { \
	...
	{  { "usec", NULL, MAVLINK_TYPE_UINT64_T, 0, 0, offsetof(mavlink_huch_sensor_array_t, usec) }, \
         { "data", NULL, MAVLINK_TYPE_FLOAT, 16, 8, offsetof(mavlink_huch_sensor_array_t, data) }, \
         } \
}
...
static inline uint16_t mavlink_msg_huch_sensor_array_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint64_t usec, const float *data)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[72];
	_mav_put_uint64_t(buf, 0, usec);
	_mav_put_float_array(buf, 8, data, 16);
        memcpy(_MAV_PAYLOAD_NON_CONST(msg), buf, 72);
#else
	mavlink_huch_sensor_array_t packet;
	packet.usec = usec;
	mav_array_memcpy(packet.data, data, sizeof(float)*16);
        memcpy(_MAV_PAYLOAD_NON_CONST(msg), &packet, 72);
#endif

	msg->msgid = MAVLINK_MSG_ID_HUCH_SENSOR_ARRAY;
	return mavlink_finalize_message(msg, system_id, component_id, 72, 50);
}
						           ...
						           uint64_t usec,const float *data)
{
	...
	msg->msgid = MAVLINK_MSG_ID_HUCH_SENSOR_ARRAY;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 72, 50);
}
...
static inline void mavlink_msg_huch_sensor_array_send(mavlink_channel_t chan, uint64_t usec, const float *data)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[72];
	_mav_put_uint64_t(buf, 0, usec);
	_mav_put_float_array(buf, 8, data, 16);
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_HUCH_SENSOR_ARRAY, buf, 72, 50);
#else
	mavlink_huch_sensor_array_t packet;
	packet.usec = usec;
	mav_array_memcpy(packet.data, data, sizeof(float)*16);
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_HUCH_SENSOR_ARRAY, (const char *)&packet, 72, 50);
#endif
}
 ...
 */
static inline uint16_t mavlink_msg_huch_sensor_array_get_data(const mavlink_message_t* msg, float *data)
{
	return _MAV_RETURN_float_array(msg, data, 16,  8);
}
...
#else
	memcpy(huch_sensor_array, _MAV_PAYLOAD(msg), 72);
#endif
}
